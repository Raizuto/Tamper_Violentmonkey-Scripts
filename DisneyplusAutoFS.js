...
// @version      1.12
// @description  To auto fullscreen Disney+ videos (it stays on) so you can binge watch all you want without using the F11 key whilst on autoplay is on. Created using the following website: https://workik.com/ai-powered-javascript-code-debugger with refining done by ChapGPT. There might be some unneeded stuff in here but it works! Warning: Currently NOT friendly with Netflix Marathon (Pausable). It is however compatiable with Dreamliner's Streaming enhanced Firefox Add-on which I use for skipping everything but Ads. For Ads I use uBlock Origin!
// @author       Raizuto
...
(function () {
  ...
  function isPlaybackStarter(el) {
    if (!el) return false;
    // If click inside the exact Continue anchor with data-testid="playback-action-button" OR aria-label "CONTINUE"
    ...
    if (playAnchor) {
      ...
      // accept if data-testid matches playback-action-button, or aria-label/text contains "continue", or href includes /play/
    ...
    // fallback: also accept common play buttons nearby
